...
router.get("/search", async (req, res, next) => {
  try {
    ...
    if (query) {
      mySearch = await Posts.find({
        $or: [{ compName: query }, { industry: query }],
      });
...
router.put("/:id/comment", async (req, res, next) => {
  try {
    const post = await Posts.findById(req.params.id);
    // post doesn't need an await because the const post already got it from the db
    post.comments.push({
      username: req.body.username,
      comment: req.body.comment,
    });
    // now you need to send it back with the save and that's why you do the await here and not on the push.
    await post.save();
    console.log(post);
    res.redirect(`/posts/${req.params.id}`);
  } catch (err) {
